...
namespace Teamwork_OOP.Engine.Items
{
	...
	public abstract class Item : CollidableObject, IBaseStats, ISecondaryStats
	{
		...
		//
		protected Item(Vector2 position, int id,float baseStatRange,float secondaryStatRange, int strength, int dexterity, float criticalDamage)
			: base(new AABB(position, new Vector2(0.5f), CollisionObjectFlags.Dynamic), id)
		{
			...
			for (int i = 0; i < addStatCount; i++)
			{
				...
				{
					case 0:
						Strength += GetRandomNumber(strength,  (int)(baseStatRange*strength));
						break;
					case 1:
						Dexterity += GetRandomNumber(dexterity, (int)(baseStatRange*dexterity));
						break;
					//todo : implement case 2 - case 14
					case 15:
